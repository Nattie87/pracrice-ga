...
for(var i=0;i<obj.placements.length;i++) {
  divId.innerHTML += '<div class="col-md-4" class="block" ><img class="image" src="' + obj.placements[i]['imageURL'] + '" style="max-width: 100px"</div>';
  divId.innerHTML += '<span class="name">' + obj.placements[i]['name'];
  divId.innerHTML += '<br>';
  divId.innerHTML += '<span class="price">' + obj.placements[i]['price'] ;
  divId.innerHTML += '</span></div>';
